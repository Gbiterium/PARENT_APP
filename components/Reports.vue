<template>
    <div class="container" style="padding-top: 6rem;">
        ...
        <div v-for="(el, index) in reports" :key="index" class="col-md-4 mb-3">
            <div class="card">
                <div class="m-3">
                    <div class="slate-dark font-weight-600 text-capitalize">{{ el.name }}</div>
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="fs-12"><span class="text-grey">Uploaded on: </span><span class="text-light-blue">{{ formatDate(el.created_on) }}</span></div>
                        <div class="icon-wrapper d-flex justify-content-center align-items-center pointer">
                            <b-spinner v-if="downloading === el" class="text-light-blue" label="Spinning"></b-spinner>
                            <b-icon-download v-else class="text-light-blue fs-20" @click.prevent="downloadFile(el)" />
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    //   try {
    //     const { data } = await $axios.get(`/util/v2/mobile/reportcards/${route.params.student}/`)
